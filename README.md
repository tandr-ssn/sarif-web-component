...
import '@acah/sarif-web-component/style.css'
...
Then open `dist/acah-viewer.html` directly in the browser. Opening the repository's root `index.html` from the filesystem redirects there as a convenience; when served over HTTP, the root page remains the Vite development shell. The build bundles React and the other runtime dependencies into that standalone file so it works offline. Generated `dist/` contents are intentionally ignored and must not be committed.
