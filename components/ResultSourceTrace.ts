...
function withPrimaryLocation(locations: Array<PhysicalLocation | undefined>, primary: PhysicalLocation): SourceTrace {
...
export function getResultSourceTrace(result: Result): SourceTrace | undefined {
	...
	if (threadFlow?.locations?.length) {
		const locations = threadFlow.locations.map(location => {
			const resolved = location.index === undefined ? location : result.run.threadFlowLocations?.[location.index]
			return resolved?.location?.physicalLocation
		})
		return {...withPrimaryLocation(locations, primary), label: 'Code flow'}
	}
